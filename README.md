...
- **`api/index.py`** — Vercel’s FastAPI entry; adds `backend/` to `sys.path` and exposes **`app`**. **`vercel.json`** **`rewrites`** send **`/api/:path*`** (e.g. **`/api/auth/login`**) to this function; otherwise Vercel only mounts **`api/index.py`** at **`/api`**, and deeper paths return platform **`NOT_FOUND`** (plain-text 404, **`x-vercel-error: NOT_FOUND`**). Non-API routes are the Vite **`frontend/dist`** shell mounted by FastAPI when **`VERCEL`** is set.
...
5. **Initial data:** When **`VERCEL=1`** and the **`users` table has zero rows**, the API runs **`seed.py`** once on startup (default users including **`benda` / `Aa123456`** plus curriculum). Set **`AUTOWASH_SKIP_VERCEL_AUTO_SEED=1`** to turn that off. If the DB already has any users, auto-seed is skipped — then run **`python seed.py`** from `backend/` against your **`DATABASE_URL`** (or add users another way) or login will fail for missing accounts. If you omit **`DATABASE_URL`**, the app uses **`/tmp` SQLite**, which is **empty on cold starts** and not shared across instances — use hosted Postgres for production.
